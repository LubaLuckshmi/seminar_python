...
def production_ready(inn_num: str):
    n_11_coefficient = (7, 2, 4, 10, 3, 5, 9, 4, 6, 8)
    n_12_coefficient = (3, 7, 2, 4, 10, 3, 5, 9, 4, 6, 8)
    n_11 = int(inn_num[-2])
    n_12 = int(inn_num[-1])
    sum_n_11 = 0
    sum_n_12 = 0
    for i in range(len(inn_num) - 2):
        sum_n_11 += int(inn_num[i]) * n_11_coefficient[i]
        sum_n_12 += int(inn_num[i]) * n_12_coefficient[i]
    sum_n_12 += int(inn_num[i+1]) * n_12_coefficient[i+1]
    div_n_11 = sum_n_11 % 11
    div_n_12 = sum_n_12 % 11
    if div_n_11 == 10:
        div_n_11 = 0
    if div_n_12 == 10:
        div_n_12 = 0
    if n_11 == div_n_11 and n_12 == div_n_12:
        return True
    return False
